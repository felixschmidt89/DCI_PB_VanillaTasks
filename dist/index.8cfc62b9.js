...
class App {
    ...
    constructor(){
        ...
        this.enterTaskElmnt.addEventListener("keydown", (e)=>{
            ...
            if (e.key === "Enter") this._addTask.bind(this)();
        });
        ...
        this.settings.addEventListener("click", this._handleSettingsClick.bind(this));
    ...
    _renderTaskList() {
        ...
        for(const taskId in this.storedTaskList){
            ...
            if (task.status !== "deleted" && (!this.hideCompleted || task.status === "open")) this._renderTaskItem(task);
        }
    }
    _clearTaskList() {
        // Remove all child nodes from the task list
        while(this.taskList.firstChild)this.taskList.removeChild(this.taskList.firstChild);
    }
    // TASKS
    _addTask() {
        ...
        if (!taskName) this._showErrorMsg();
        else {
            this._clearErrorMsg();
    ...
    _toggleTaskCompletion(taskId) {
        const checkbox = document.querySelector(`.tasks-task[data-id="${taskId}"] input[type="checkbox"]`);
        if (checkbox) {
            const liElement = checkbox.closest(".tasks-task");
    ...
    _handleTaskCompletion(taskId) {
        if (this.storedTaskList.hasOwnProperty(taskId)) {
            const task = this.storedTaskList[taskId];
            // Toggle completion status
            if (task.status === "completed") // Task was completed, mark as open
            task.status = "open";
            else // Task was open, mark as completed
    ...
    _changeTaskName(taskId) {
        const liElement = document.querySelector(`.tasks-task[data-id="${taskId}"]`);
    ...
    handleDrop(e) {
        ...
        if (sourceElement && targetElement && sourceElement !== targetElement) {
            ...
            if (sourceIndex !== -1 && targetIndex !== -1) this.moveTask(sourceIndex, targetIndex);
        }
    ...
    /* SETTINGS AND ADDITIONAL FUNCTIONALITY CONTAINER*/ _showToolTips(index) {
    ...
    _handleSettingsClick(e) {
        ...
        if (e.target.matches(".fa-dumpster-fire")) {
            ...
            const confirmed = confirm("Do you really want to delete your entire task list and all related data?");
            if (confirmed) this._deleteAllTasks();
        } else if (e.target.matches(".fa-lightbulb-o")) // Toggle dark mode if the lightbulb icon is clicked
        this._toggleDarkMode();
        else if (e.target.matches(".fa-eye")) // Toggle visibility of completed tasks if the eye icon is clicked
...
},{}]},["gAoaA","6rimH"], "6rimH", "parcelRequired803")
